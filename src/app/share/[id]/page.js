...
export default function PublicSharePage() {
  ...
  const getAbyssEmbedUrl = (abyssId) => {
    if (!abyssId) return "";
    // Abyss.to এর সাধারণ embed format (তাদের ডকস অনুযায়ী প্রয়োজনে পরিবর্তন করবেন)
    return `https://abyssplayer.com/${abyssId}`; 
  };

  const getDirectDownloadUrl = (url) => {
    if (!url || url === "Failed" || url === "Error") return "#";
    const match = url.match(/[-\w]{25,}/);
    if (match && match[0]) {
      const driveId = match[0].trim();
      try {
        const encodedId = btoa(driveId);
        return `https://d.instantdl.cfd/dokan/directcloud.php/${encodedId}`;
      } catch (e) {
        return url;
      }
    }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f4f7f6" }}>
        <div style={{ color: "#3b82f6", fontSize: "20px", fontWeight: "bold", animation: "pulse 1.5s infinite" }}>
          Loading MovieNewsBD Server...
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="share-layout">
      {/* আপনার আগের CSS স্টাইলগুলো এখানে থাকবে, আমি আর রিপিট করলাম না জায়গা বাঁচানোর জন্য */}
      <style dangerouslySetInnerHTML={{ __html: `
        /* আপনার আগের সব CSS স্টাইল হুবহু এখানে বসিয়ে দিবেন */
        ...
        @keyframes pulseDot { 0% { opacity: 1; transform: scale(1); box-shadow: 0 0 0 0 rgba(34, 197, 94, 0.7); } 50% { opacity: 0.4; transform: scale(0.9); box-shadow: 0 0 10px 4px rgba(34, 197, 94, 0.2); } 100% { opacity: 1; transform: scale(1); box-shadow: 0 0 0 0 rgba(34, 197, 94, 0); } }
        ...
        .btn-instant { background: #5b21b6; } .btn-dldokan { background: #1e293b; } .btn-gocloud { background: #0f766e; } .btn-drivecloud { background: #2563eb; } .btn-gcloud { background: #c026d3; }
        .badge { font-size: 10px; padding: 3px 8px; border-radius: 5px; font-weight: 800; white-space: nowrap; text-transform: uppercase; }
        .badge-dark { background: #000; color: white; border: 1px solid rgba(255,255,255,0.2); } .badge-red { background: #ef4444; color: white; }
        ...
        @media (max-width: 600px) { .share-layout { padding: 10px; } .main-card { border-radius: 12px; } .watch-box { margin: 15px; padding: 12px 15px; font-size: 15px; border-radius: 10px; } .play-btn { padding: 6px 15px; font-size: 13px; gap: 5px; } .dl-btn { font-size: 13px; padding: 12px 14px; flex-direction: row; } .btn-left { font-size: 12px; gap: 6px; } .btn-right { gap: 6px; } .badge { font-size: 9px; padding: 2px 6px; } .title-box { font-size: 15px; padding: 15px; } .info-row { font-size: 13px; padding: 12px 15px; } .brand-title { font-size: 14px; } .status-dot { height: 10px; width: 10px; } }
      `}} />
      ...
      {isPlaying && (
        <div className="player-modal">
          <div className="player-header">
            <button className="close-player-btn" onClick={() => setIsPlaying(false)}>
              ✖ Close
            </button>
          </div>
          <iframe
            src={getAbyssEmbedUrl(movie.abyssId)} // 🟢 ড্রাইভের বদলে Abyss এর লিংক 
      ...
      <div className="main-card">
        ...
        {/* 🎬 Watch Box - এখন Abyss.to এর উপর নির্ভরশীল */}
        <div
          className="watch-box"
          onClick={() => {
        ...
        {/* নিচের সব ডাউনলোড বাটন আগের মতই থাকবে */}
